...
vector<int> * Renderer::findClosestFace(Vector3<double> source, Vector3<double> destination, const std::vector<Face *> & faces, const vector<Light *> & lights, int depth, double * distance, bool * found) {
	...
	for (size_t i = 0; i < faces.size(); i++) {
		...
		if (faces[i]->isInside(temp)) {
			if (T > 0) {
				current_distance = (temp - source).magnitude();
...
vector<int> * Renderer::colorFacePixel(Vector3<double> source, Vector3<double> destination, Face * face, const vector<Face *> &faces, const std::vector<Light * > & lights, int depth) {
	//cout << "got here" << endl;   
	...
	*/
	if (depth > 1) {
		const Vector3<double>  *normal = face->getNormal();
		Vector3<double> L = source - destination;
		Vector3<double> scl = normal->scale(2 * L.dot(*normal));
		Vector3<double> new_dest = (scl - L) + destination;
		...
		vector<int> * reflective = findClosestFace(destination, new_dest, faces, lights, depth - 1, &f_dist, &found_face);
	...
	for (size_t j = 1; j < lights.size(); j++) {
		if (!lights[j]->isBlocked(destination, faces)) {
			...
			if (((c < 0) && (awesome < 0)) || ((c > 0) && (awesome > 0))) {
				// cout << "past c" << endl;
				vector<int> * color = face->colorDiffuse(destination, lights[j]->get_color(), c);
				r += (*color)[0];
				g += (*color)[1];
				b += (*color)[2];
				delete(color);
...
void Renderer::prepareRaycast(const vector<Face *> & faces, const vector<Light * > & lights) {
	...
	for (int grid_y = min_y; grid_y < max_y - 1; grid_y++) {
		for (int grid_x = min_x; grid_x < max_x - 1; grid_x++) {
			Vector3<double> pixel = Vector3<double>(
			camPos[0] + (u[0])*(grid_x)*(1 / abs(max_x)) + (v[0])*(grid_y)*(1 / abs(max_y)),
			camPos[1] + u[1] * grid_x*(1 / abs(max_x)) + v[1] * grid_y*(1 / abs(max_y)),
			camPos[2] + u[2] * grid_x*(1 / abs(max_x)) + v[2] * grid_y*(1 / abs(max_y)));
		...
		/*	cout << "testing" << endl;
			...
			f_color = findClosestFace(fp, pixel, faces, lights, depth, &f_dist, &found_face);
