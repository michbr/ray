...
bool Mesh::intersects(const Ray3<double> &ray) const {
...
double Mesh::intersection(const Ray3<double> &ray) const {
	double min = DBL_MAX;
...
Collision Mesh::collision(const Ray3<double> &ray) const {
	Collision min;
	for(size_t i=0; i<groups.size(); ++i) {
...
bool TriangleGroup::intersects(const Ray3<double> &ray, const Mesh &parent) const {
	return false;
}

double TriangleGroup::intersection(const Ray3<double> &ray, const Mesh &parent) const {
	return 1;
}

Collision TriangleGroup::collision(const Ray3<double> &ray, const Mesh &parent) const {
	Collision col;
	...
	for(unsigned short i=0; i<end; ++i) {
		...
		if (surfNormals[i].dot(ray.direction) >= 0)
			continue;
		
		// get base vertex and two edges from it
		unsigned short vertInd = i*3;
		Vector3<double> vertex1 = parent.vertices[indices[vertInd]];
		Vector3<double> edge1 = parent.vertices[indices[vertInd+1]] -vertex1;
		Vector3<double> edge2 = parent.vertices[indices[vertInd+2]] -vertex1;
		
		// calculate first intermediate value
		Vector3<double> h = ray.direction.cross(edge2);
		double a = edge1.dot(h);
		if (a > -0.0001 && a < 0.0001)
			continue;
		
		// calculate the "x" location of the collision on the triangle
		Vector3<double> diff = ray.position -vertex1;
		double planeX = diff.dot(h) /a;
		if (planeX < 0 || planeX > 1)
			continue;
		
		// calculate the "y" location of the collision on the triangle
		Vector3<double> q = diff.cross(edge1);
		double planeY = ray.direction.dot(q) /a;
		...
		if (distance < col.distance && distance >= 0) {
			...
			const Vector3<double> &norm1 = parent.vertNormals[indices[vertInd]];
			const Vector3<double> &norm2 = parent.vertNormals[indices[vertInd+1]];
			const Vector3<double> &norm3 = parent.vertNormals[indices[vertInd+2]];
