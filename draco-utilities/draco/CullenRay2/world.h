...
class World {
	public:
		...
		Color trace(const Ray3<double> &ray, double depth=0, double minDistance=0, double maxDistance=DBL_MAX, double *distance=NULL) const;
		
		// returns the distance along the ray before a collision with an object in the world model
		double intersection(const Ray3<double> &ray, double minDistance=0, double maxDistance=DBL_MAX) const;
	...
	private:
		...
		Color calculateLight(const Ray3<double> &ray, const Collision &col) const;
};
