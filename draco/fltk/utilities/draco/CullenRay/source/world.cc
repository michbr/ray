...
Vec3 calcRefraction(const Vec3 &direction, const Vec3 &normal, double mu) {
...
void World::render(char *commandFilename) {
	...
	for(Camera cam: cameras) {
		...
		Vec3 zDir = cam.normal.normal();
		Vec3 xDir = cam.up.cross(zDir).normal();
		...
		Vec2 viewPlaneSize(2.0 *((double)xRes /(double)yRes), 2);
		Vec2 viewPlaneOffset(-viewPlaneSize.x /2, -viewPlaneSize.y /2);
		Vec2 pixelMult(viewPlaneSize.x /(xRes -1), viewPlaneSize.y /(yRes -1));
		...
		for(int y=0; y<yRes; ++y) {
			for(int x=0; x<xRes; ++x) {
				...
				Vec2 gridPos(x *pixelMult.x +viewPlaneOffset.x, y *pixelMult.y +viewPlaneOffset.y);
				Vec3 pix = Vec3(gridPos.x, gridPos.y, cam.near).normal();
				ray.vector = pix.x *xDir +pix.y *yDir -pix.z *zDir;
				double distance;
...
Color World::trace(const Ray3 &ray, double depth, double minDistance, double maxDistance, double *distance) const {
	Collision col;
	...
	Vec3 norm = (col.normal.dot(ray.vector) > 0)? -col.normal: col.normal;
	double extraDis = 0;
	if (norm != Vec3::zero) {
		...
		if (depth > 1 && ref > 0) {
			Ray3 newRay;
			Vec3 intermediate = -ray.vector.dot(norm) *norm;
			newRay.vector = intermediate *2 +ray.vector;
			newRay.position = col.position +newRay.vector *0.01;
			double tempDis;
		...
		if (depth > 1 && alpha < 1) {
			...
			if (sphere == NULL) {
				...
				Vec3 T = calcRefraction(ray.vector, norm, index);
				if (T.lengthSquared() != 0) {
					Ray3 newRay(col.position, T);
			...
			// refract through a polygon (only works properly if the bakcside culling is disabled)
				...
				Vec3 T = calcRefraction(ray.vector, norm, 1 /mat.refractionIndex);
				if (T.lengthSquared() != 0) {
					Ray3 newRay;
					newRay.position = col.position +(sphere->position -col.position).dot(T) *T *2;
					newRay.vector = calcRefraction(T, (sphere->position -newRay.position) /sphere->radius, mat.refractionIndex);
					newCol = trace(newRay, depth -REFRACTION_DEPTH_PENALTY, 0, DBL_MAX, &tempDis);
				}
			}
...
Color World::calculateLight(const Ray3 &ray, const Collision &col) const {
	Color colorPixel(0, 0, 0);
	...
	for(list<LightSource>::const_iterator it=lights.cbegin(); it!=lights.cend(); ++it) {
		...
		if (intersection(Ray3(col.position +toLight, -lightDir), 0, distance -0.001) < distance -0.001)
			continue;
		...
		Vec3 intermediate = lightDir.dot(norm) *norm;
		Vec3 bounceDir = (intermediate -lightDir) *2 +lightDir;
		double cosign = -bounceDir.dot(ray.vector);
		if (cosign < 0)
			continue;
...
double World::intersection(const Ray3 &ray, double minDistance, double maxDistance) const {
