...
public static class MarchingCubes {
	...
	static Hashtable vertices;
	static int size;
	static int[] vertlist = new int[12];
	static float isolevel, voxelSize;
	static float[,,] voxels;

	/*
	 * MUST BE CALLED BEFORE Polygonize!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	 */
	public static void setup(int size, float voxelSize, float isolevel, ref Hashtable vertices, ref float[,,] voxels) {
		MarchingCubes.size = size;
	...
	*/
	public static int[] polygonise(int x, int y, int z, int x1, int y1, int z1) {
		/*
		...
		*/
		...
		if ((edgeTable[cubeindex] & 1) != 0) {
			...
			VertexInterp(x, y, z, x1, y, z, voxels[x, y, z], voxels[x1, y, z], vertlist[0]);
		}
		if ((edgeTable[cubeindex] & 2) != 0) {
			vertlist[1] = ((size - 1 + z) * size + y) * size + x1;
			VertexInterp(x1, y, z, x1, y, z1, voxels[x1, y, z], voxels[x1, y, z1], vertlist[1]);
		}
		if ((edgeTable[cubeindex] & 4) != 0) {
			vertlist[2] = (x * size + y) * size + z1;
			VertexInterp(x1, y, z1, x, y, z1, voxels[x1, y, z1], voxels[x, y, z1], vertlist[2]);
		}
		if ((edgeTable[cubeindex] & 8) != 0) {
			vertlist[3] = ((size - 1 + z) * size + y) * size + x;
			VertexInterp(x, y, z1, x, y, z, voxels[x, y, z1], voxels[x, y, z], vertlist[3]);
		}
		if ((edgeTable[cubeindex] & 16) != 0) {
			vertlist[4] = (x * size + y1) * size + z;
			VertexInterp(x, y1, z, x1, y1, z, voxels[x, y1, z], voxels[x1, y1, z], vertlist[4]);
		}
		if ((edgeTable[cubeindex] & 32) != 0) {
			vertlist[5] = ((size - 1 + z) * size + y1) * size + x1;
			VertexInterp(x1, y1, z, x1, y1, z1, voxels[x1, y1, z], voxels[x1, y1, z1], vertlist[5]);
		}
		if ((edgeTable[cubeindex] & 64) != 0) {
			vertlist[6] = (x * size + y1) * size + z1;
			VertexInterp(x1, y1, z1, x, y1, z1, voxels[x1, y1, z1], voxels[x, y1, z1], vertlist[6]);
		}
		if ((edgeTable[cubeindex] & 128) != 0) {
			vertlist[7] = ((size - 1 + z) * size + y1) * size + x;
			VertexInterp(x, y1, z1, x, y1, z, voxels[x, y1, z1], voxels[x, y1, z], vertlist[7]);
		}
		if ((edgeTable[cubeindex] & 256) != 0) {
			vertlist[8] = ((size *2 - 2 + y) * size + x) * size + z;
			VertexInterp(x, y, z, x, y1, z, voxels[x, y, z], voxels[x, y1, z], vertlist[8]);
		}
		if ((edgeTable[cubeindex] & 512) != 0) {
			vertlist[9] = ((size * 2 - 2 + y) * size + x1) * size + z;
			VertexInterp(x1, y, z, x1, y1, z, voxels[x1, y, z], voxels[x1, y1, z], vertlist[9]);
		}
		if ((edgeTable[cubeindex] & 1024) != 0) {
			vertlist[10] = ((size * 2 - 2 + y) * size + x1) * size + z1;
			VertexInterp(x1, y, z1, x1, y1, z1, voxels[x1, y, z1], voxels[x1, y1, z1], vertlist[10]);
		}
		if ((edgeTable[cubeindex] & 2048) != 0) {
			vertlist[11] = ((size * 2 - 2 + y) * size + x) * size + z1;
			VertexInterp(x, y, z1, x, y1, z1, voxels[x, y, z1], voxels[x, y1, z1], vertlist[11]);
		}
	...
	*/
	private static void VertexInterp(int x1, int y1, int z1, int x2, int y2, int z2, float valp1, float valp2, int vox) {
		if (Mathf.Abs(isolevel-valp1) < 0.00001) {
			vertices[vox] = new Vector3(x1 * voxelSize, y1 * voxelSize, z1 * voxelSize);
			return;
		}
		if (Mathf.Abs(isolevel - valp2) < 0.00001) {
			vertices[vox] = new Vector3(x2 * voxelSize, y2 * voxelSize, z2 * voxelSize);
			return;
		}
